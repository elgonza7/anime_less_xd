...
export default function CompareCard({ side, imageUrl, label, subLabel, revealed, isWinner, disabled, onClick }) {
  ...
  return (
    <motion.button
      ...
      className={`group relative h-[26rem] w-full overflow-hidden rounded-3xl border-2 bg-panel text-left shadow-xl transition-colors sm:h-[36rem] sm:w-[26rem] ${borderClass} ${
    ...
    >
      ...
      <AnimatePresence>
        {revealed && isWinner && (
          <motion.div
            ...
            className="absolute right-4 top-4 flex h-12 w-12 items-center justify-center rounded-full bg-emerald-500 text-2xl text-white shadow-lg"
          >
            ✓
          </motion.div>
        )}
      ...
      <div className="absolute inset-x-0 bottom-0 space-y-1.5 p-5">
        <p className="text-xl font-black leading-tight text-white drop-shadow sm:text-2xl">{label}</p>
        <p
          className={`text-base font-bold transition-opacity ${revealed ? "opacity-100" : "opacity-0"} ${
