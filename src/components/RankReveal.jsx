...
function Row({ rank, displayName, photoURL, totalPoints, highlight, delay, climbIn }) {
  return (
    <motion.div
      ...
      className={`flex items-center gap-3 rounded-xl px-4 py-2.5 ${
        highlight ? "bg-violet-600/20 ring-2 ring-violet-500" : "bg-panel"
      }`}
    >
      ...
      {highlight && <span className="text-xs font-bold uppercase text-violet-400">you</span>}
    </motion.div>
  );
}
...
export default function RankReveal({ myRank, top, myUid }) {
  const isInTop = top.some((row) => row.uid === myUid);
      ...
      <div className="flex w-full max-w-sm flex-col gap-2">
        ...
        {!isInTop && (
          <>
            ...
            <Row {...myRank} highlight delay={top.length * 0.15 + 0.2} climbIn />
          </>
        )}
      </div>
    </div>
